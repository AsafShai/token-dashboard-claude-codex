...
def _migrate_add_source(conn) -> None:
    """Add messages.source to distinguish Claude vs Codex turns."""
    has_table = conn.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='messages'"
    ).fetchone()
    if not has_table:
...
def _source_clause(source, alias: str = ""):
...
def overview_totals(db_path, since=None, until=None, source=None) -> dict:
    ...
    with connect(db_path) as c:
        return dict(c.execute(sql, args + src_args).fetchone())
...
def expensive_prompts(db_path, limit: int = 50, sort: str = "tokens", source=None) -> list:
    ...
    src, src_args = _source_clause(source, alias="u")
    sql = f"""
